...
void Excersize_2_23(int solution, int answer) //한라대학교 미래모빌리티공학과 김태민
{
	...
	if (solution == SHOW)
	{
		...
		printf("등가속도 운동 공식은 Δy = v0 * t + 1/2 * a * t^2 와 v^2 - v0^2 = 2 * a * Δy 가 있다.\n");
		printf("먼저 Δy = v0 x t + 1/2 x a x t^2를 활용하여 창문의 꼭대기에서의 속도를 구해주기위해 v0를 구해준다.\n\n");
		printf("Δy:위치의 변화량,v0:처음 속도, a:가속도 , t:시간\n");
		printf("Δy는 위치의 변화량으로 창문의 높이인 %5.2lf m 이다.\n", height);
		printf("자유낙하운동이므로 가속도 a가 중력가속도 g로 변한다.\n");
		printf("가속도 a: -%5.2lfm/s^2\n", gravitational_acceleration);
		printf("t는 돌이 창문 꼭대기부터 떨어지는 시간으로 %5.2lf s 이다.\n", time);
		printf("Δy, g, t를 위 식에 대입하여 v0를 구해줍니다.\n");
		printf("위 식에 대입하면 %5.2lf m = v0 * %5.2lf s + 1/2 * -%5.2lf m/s^2 * (%5.2lf s)^2 이다.\n", height, time, gravitational_acceleration, time);
		printf("v0의 값을 구한다.\n\n\n");
		...
		printf("위 식에 대입하면 %5.3lf - (%5.2lf)^2 = 2 * %5.2lf * Δy이다.\n", spe3, (v0 * v0), gravitational_acceleration);
		printf("Δy의 값을 구한다.\n");
	...
	if (answer == SHOW)
	{
		...
		printf("v0의 값은 다음과 같습니다. = %5.3lfm \n\n", spe3);
		v0 = v0 * time * 0.5 * gravitational_acceleration * (time * time);

        printf("Δy의 값은 다음과 같습니다. = %5.2lfm\n", spe5);
		Δy = spe3 / 2 * gravitational_acceleration;
